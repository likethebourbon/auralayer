...
class Segment {
	...
	create_segment() {
		...
		this.TextEditingMenuContainer_SingleSegment = createNewElement({
			type: "div",
			classes: ["TextEditingMenuContainer_SingleSegment"],
			parent: this.segment,
			properties: {},
			styles: { display: "none" },
		});
	...
	click_handler(e) {
		...
		[...this.parent.parent.TextEditingMenuContainer.children].forEach(
...
class Auralayer {
	...
	initialize_interface() {
		...
		this.HeaderRow = createNewElement({
			type: "div",
			classes: ["d-flex", "flex-row", "my-1", "justify-content-stretch"],
		...
		this.HeaderSettingsGearButton = createNewElement({
			...
			dataset: { bsToggle: "offcanvas", bsTarget: "#offcanvasExample" },
		...
		this.BodyContainer = createNewElement({
			type: "div",
			classes: ["BodyContainer", "container-xl"],
			parent: this.Body,
		});
		...
		this.PresenceSliderEndLabel = createNewElement({
			...
			classes: ["form-label"],
			parent: this.PresenceSliderContainer,
			properties: { for: "presence_end", innerText: "Presence (end)" },
		});
		...
		this.TextEditingMenuContainer = createNewElement({
			type: "div",
			classes: ["TextEditingMenuContainer"],
		...
		this.TextEditingLeftAlignButton = createNewElement({
			...
			parent: this.TextEditingMenuContainer,
			properties: { innerHTML: `<i class="bi-justify-left"></i>` },
			events: {
				click: (e) => {
					this.ChangeTextFormat({
		...
		this.DataAccordionButton = createNewElement({
			type: "button",
			classes: ["DataAccordionButton", "accordion-button", "collapsed"],
		...
		this.DataTableWrapper = createNewElement({
			type: "div",
			classes: ["col-lg-8", "offset-lg-2", "table-responsive"],
		...
		this.VideoAccordionButton = createNewElement({
			type: "button",
			classes: ["VideoAccordionButton", "accordion-button", "collapsed"],
		...
		for (let i = 0; i < this.layers.length; i++) {
			for (let j = 0; j < this.layers[i].segment_array.length; j++) {
				...
				this.layers[i].segment_array[
					j
				].TextEditingMenuContainer_SingleSegment.style.display = "none";
			}
		}

		// this.TextEditingMenuContainer.style.display = "none";
		// this.TextEditingMenuContainer.style.cursor = "not-allowed";
		[...this.TextEditingMenuContainer.children].forEach(
			(each) => (each.style.cursor = "not-allowed")
		);
	}
	deselect_all_layers() {
