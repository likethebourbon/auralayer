...
class Layer {
	...
	initialize() {
		let r = parseInt(
			this.layer_data.color.split("rgba(")[1].split(",")[0]
		).toString(16);
		let g = parseInt(
			this.layer_data.color.split("rgba(")[1].split(",")[1]
		).toString(16);
		let b = parseInt(
			this.layer_data.color.split("rgba(")[1].split(",")[2]
		).toString(16);
		...
		this.layer_container.addEventListener("dragstart", (e) => {
		...
		this.layer_segment_holder = createNewElement({
			...
			parent: this.layer_container,
			styles: {
				width: this.parent_file_length * this.parent.scale + "px",
			},
		});
		...
		this.select_box_selector_box = createNewElement({
			type: "div",
			classes: ["select_box_selector_box"],
			parent: this.layer_controls_holder,
			properties: {},
		});
		this.select_box_selector_box.addEventListener("click", (e) =>
			this.select_box.click()
		);
		this.color_picker = createNewElement({
			type: "input",
			classes: ["layer_color_picker", "layer_controls_elements"],
			parent: this.layer_controls_holder,
			properties: { type: "color", value: "#" + r + g + b },
			styles: { display: "none" },
		});
		...
		this.name = createNewElement({
			...
			parent: this.layer_controls_holder,
			properties: { innerHTML: this.layer_data.name },
		});
		this.name.addEventListener("dblclick", (e) =>
			this.layer_name_double_click_handler(e)
		);
		...
		this.name.addEventListener(
			"blur",
			(e) => (this.name.contentEditable = false)
		);
		...
		this.layer_texture_picker = createNewElement({
			type: "button",
			classes: ["layer_texture_picker", "layer_controls_elements"],
			parent: this.layer_controls_holder,
			properties: { innerText: "texture" },
			styles: { display: "none" },
		});
		...
		this.shape_background_texture_1 = createNewElement({
			type: "button",
			classes: ["shape_background_texture_1"],
		...
		this.shape_background_texture_2 = createNewElement({
			type: "button",
			classes: ["shape_background_texture_2"],
			parent: this.texture_selector,
		...
		this.shape_background_texture_3 = createNewElement({
			type: "button",
			classes: ["shape_background_texture_3"],
			parent: this.texture_selector,
		...
		this.shape_background_texture_4 = createNewElement({
			type: "button",
			classes: ["shape_background_texture_4"],
			parent: this.texture_selector,
		...
		this.shape_background_texture_5 = createNewElement({
			type: "button",
			classes: ["shape_background_texture_5"],
			parent: this.texture_selector,
		...
		this.shape_background_texture_6 = createNewElement({
			type: "button",
			classes: ["shape_background_texture_6"],
			parent: this.texture_selector,
		...
		this.shape_background_texture_7 = createNewElement({
			type: "button",
			classes: ["shape_background_texture_7"],
			parent: this.texture_selector,
		...
		this.shape_background_texture_8 = createNewElement({
			type: "button",
			classes: ["shape_background_texture_8"],
			parent: this.texture_selector,
		...
		this.shape_background_texture_9 = createNewElement({
			type: "button",
			classes: ["shape_background_texture_9"],
			parent: this.texture_selector,
	...
	create_layer_background_texture(e) {
		for (let i = 0; i < this.segment_array.length; i++) {
			...
			this.segment_array[i].segment.style.background =
				e.target.style.background + " center center, " + initial_value;
		...
		if (this.mode === "load_existing_layer") {
			this.segment_array.push(
				new Segment(
					...
					this.parent.presence_slider_start,
					this.parent.presence_slider_end,
					start_presence,
					end_presence,
					sent_segment
				)
			...
			let current_position_relative_to_current_start_pos =
				...
				this.layer_data.segments[this.current_segment_index].start_pos;
			...
			this.segment_array[this.current_segment_index].segment.style.width =
				new_width_of_current_segment * this.parent.scale - 1 + "px";
	...
	create_segment() {
		this.segment = createNewElement({
			...
			styles: {
				...
				width:
					(this.data.end_pos - this.data.start_pos) *
						this.parent.parent.scale +
					(this.parent.parent.scale - 1) +
					"px",
		...
		this.segment.addEventListener("dblclick", (e) => {
			this.segment_double_click_handler();
		});
		this.segment.addEventListener("input", (e) =>
			this.segment_text_input_handler(e)
		);
		this.segment.addEventListener(
			"blur",
			(e) => (this.segment.contentEditable = false)
		);
	}
	...
	click_handler() {
		...
		if (shift_down === false) {
			this.parent.parent.body
				.querySelectorAll(".segment_selected")
				.forEach((each) => each.classList.remove("segment_selected"));
		}
		...
		if (deselect === true) {
			...
			let second_color_saturation_value = this.data.color
				...
				.slice(-5)
				.slice(0, 3);
			this.presence_slider_start.value = parseInt(
				first_color_saturation_value * GLOBAL_presence_scale
			);
			this.presence_slider_end.value = parseInt(
				second_color_saturation_value * GLOBAL_presence_scale
			);
			this.data.start_presence = parseInt(
				this.presence_slider_start.value
			);
...
class Auralayer {
	...
	create_activity_selection_interface() {
		this.activity_selection_container = createNewElement({
			type: "div",
			classes: ["activity_selection_container"],
			parent: document.body,
		});
		this.activity_selection_header = createNewElement({
			type: "div",
			classes: ["activity_selection_header"],
			parent: this.activity_selection_container,
			properties: { innerText: "Auralayer" },
		});
		this.activity_selection_body = createNewElement({
			type: "div",
			classes: ["activity_selection_body"],
			parent: this.activity_selection_container,
		});
		this.activity_selection_footer = createNewElement({
			type: "div",
			classes: ["activity_selection_footer"],
			parent: this.activity_selection_container,
		});

		this.new_auralayer_from_youtube_container = createNewElement({
			type: "div",
			classes: [
				"new_auralayer_from_youtube_container",
				"activity_button_container",
			],
			parent: this.activity_selection_body,
		});
		this.new_auralayer_from_youtube_button = createNewElement({
			type: "button",
			classes: ["new_auralayer_from_youtube_button","btn","btn-primary"],
			parent: this.new_auralayer_from_youtube_container,
			properties: { innerText: "Create" },
		});
		this.new_auralayer_from_youtube_description = createNewElement({
			type: "div",
			classes: ["new_auralayer_from_youtube_description"],
			parent: this.new_auralayer_from_youtube_container,
			properties: {
				innerText: "Create a new Auralayer using a YouTube link",
			},
		});
		this.new_auralayer_from_youtube_button.addEventListener(
			"click",
			(e) => {
		...
		this.new_auralayer_from_audio_file_container = createNewElement({
			type: "div",
			classes: [
				"new_auralayer_from_audio_file_container",
				"activity_button_container",
			],
			parent: this.activity_selection_body,
		});
		this.new_auralayer_from_audio_file_button = createNewElement({
			type: "button",
			classes: ["new_auralayer_from_audio_file_button","btn","btn-primary"],
			parent: this.new_auralayer_from_audio_file_container,
			properties: { innerText: "Create" },
		});
		this.new_auralayer_from_audio_file_description = createNewElement({
			type: "div",
			classes: ["new_auralayer_from_audio_file_description"],
			parent: this.new_auralayer_from_audio_file_container,
			properties: {
				innerText:
		...
		this.new_auralayer_from_audio_file_button.addEventListener(
			"click",
			(e) => this.start_audio_file_activity_setup()
		);

		this.open_existing_auralayer_from_file_container = createNewElement({
			type: "div",
			classes: [
				"open_existing_auralayer_from_file_container",
				"activity_button_container",
			],
			parent: this.activity_selection_body,
		});
		this.open_existing_auralayer_from_file_button = createNewElement({
			type: "button",
			classes: ["open_existing_auralayer_from_file_button","btn","btn-primary"],
			parent: this.open_existing_auralayer_from_file_container,
			properties: { innerText: "Open" },
		});
		this.open_existing_auralayer_from_file_description = createNewElement({
			type: "div",
			classes: ["open_existing_auralayer_from_file_description"],
			parent: this.open_existing_auralayer_from_file_container,
			properties: {
				innerText:
		...
		this.open_existing_auralayer_from_file_button.addEventListener(
			"click",
			(e) => this.import_from_file.click()
		);

		this.import_from_file = createNewElement({
			type: "input",
			classes: ["interface_button"],
			parent: document.body,
			properties: { type: "file" },
			styles: { display: "none" },
		});
		this.import_from_file.addEventListener("change", (e) =>
			this.request_file_from_user(e)
		);

		this.new_auralayer_from_audio_file_with_absolute_URL_container =
			createNewElement({
				type: "div",
				classes: [
					"new_auralayer_from_audio_file_with_absolute_URL_container",
					"activity_button_container",
				],
				parent: this.activity_selection_body,
				styles: { display: "none" },
			});
		this.new_auralayer_from_audio_file_with_absolute_URL_button =
			createNewElement({
				type: "button",
				classes: [
					"new_auralayer_from_audio_file_with_absolute_URL_button",
					"btn",
					"btn-primary",
				],
				parent: this
		...
		this.new_auralayer_from_audio_file_with_absolute_URL_description =
			createNewElement({
				type: "div",
				classes: [
					"new_auralayer_from_audio_file_with_absolute_URL_description",
				],
				parent: this
					.new_auralayer_from_audio_file_with_absolute_URL_container,
				properties: {
		...
		if (location.hostname.includes("localhost")) {
			this.new_auralayer_from_audio_file_with_absolute_URL_container.style.display =
				"flex";
		}
	}
	initialize_interface() {
		this.auralayer_program = createNewElement({
			type: "div",
			classes: ["auralayer_program"],
			parent: document.body,
		});
		...
		this.header = createNewElement({
			type: "div",
			classes: ["header_al"],
			parent: this.auralayer_program,
		});
		this.layout_wrapper = createNewElement({
			type: "div",
			classes: ["row", "flex-wrap", "layout_wrapper"],
			parent: this.auralayer_program,
		});
		this.left_outer_column = createNewElement({
			type: "div",
			classes: ["left_outer_column", "col", "col-md-3","text-center"],
			parent: this.layout_wrapper,
		});
		this.body = createNewElement({
			type: "div",
			classes: ["body_al", "col-md"],
			parent: this.layout_wrapper,
		});
		this.right_outer_column = createNewElement({
			type: "div",
			classes: ["right_outer_column"],
			parent: this.layout_wrapper,
		});
		...
		// -----------------------------------
		this.all_layer_containers = createNewElement({
			type: "div",
			classes: ["all_layer_containers", "row"],
			parent: this.body,
		});
		this.all_layer_containers.addEventListener("dragover", (e) => {
			this.dragging_handler(e);
		});
		this.segment_editing_container = createNewElement({
			type: "div",
			classes: ["segment_editing_container", "row", "flex-wrap"],
			parent: this.body,
		});
		this.slider_container = createNewElement({
			type: "div",
		...
		// -----------------------------------
		this.seek_slider = createNewElement({
			type: "input",
			classes: ["slider", "seek_slider", "form-range"],
			parent: this.slider_container,
			properties: { type: "range" },
		});
		this.seek_slider.addEventListener("input", (e) =>
		...
		// -----------------------------------
		this.left_button_group = createNewElement({
			type: "div",
			role: "group",
			classes: ["btn-group-vertical"],
			parent: this.left_outer_column,
		});
		this.add_layer_button = createNewElement({
			type: "button",
			classes: ["add_layer_button", "btn", "btn-primary"],
			parent: this.left_button_group,
			properties: { innerText: "+ Add Layer" },
		});
		this.add_layer_button.addEventListener("click", (e) =>
			this.add_layer_handler()
		);

		this.save_to_file_button = createNewElement({
			type: "button",
			classes: ["save_to_file_button", "btn", "btn-primary"],
			parent: this.left_button_group,
			properties: { innerText: "Save" },
		});
		this.save_to_file_button.addEventListener("click", (e) => {
			this.save_to_file();
		});

		this.zoom_in_button = createNewElement({
			type: "button",
			classes: ["zoom_in_button", "btn", "btn-primary"],
			parent: this.left_button_group,
			properties: { innerText: "Zoom In" },
		});
		this.zoom_in_button.addEventListener("click", (e) => {
			this.zoom_handler("in");
		});
		this.zoom_out_button = createNewElement({
			type: "button",
			classes: ["zoom_out_button", "btn", "btn-primary"],
			parent: this.left_button_group,
			properties: { innerText: "Zoom Out" },
		});
		this.zoom_out_button.addEventListener("click", (e) => {
		...
		// -----------------------------------
		this.segment_button_group = createNewElement({
			type: "div",
			role: "group",
			classes: ["segment_button_group", "btn-group"],
			parent: this.body,
		});
		this.split_button = createNewElement({
			type: "button",
			classes: ["split_button", "btn", "btn-outline-secondary"],
			parent: this.segment_button_group,
			properties: { innerText: "Split" },
		});
		this.split_button.addEventListener("click", (e) =>
			this.split_selected_segment(e)
		);
		this.merge_left_button = createNewElement({
			type: "button",
			classes: ["merge_left_button", "btn", "btn-outline-secondary"],
			parent: this.segment_button_group,
			properties: { innerText: "← Merge" },
		});
		this.merge_left_button.addEventListener("click", (e) =>
			this.merge_segments(e, "left")
		);
		this.delete_button = createNewElement({
			type: "button",
			classes: ["delete_button", "btn", "btn-outline-danger"],
			parent: this.segment_button_group,
			properties: { innerText: "Delete" },
		});
		this.delete_button.addEventListener("click", (e) =>
			this.delete_button_handler(e)
		);
		this.merge_right_button = createNewElement({
			type: "button",
			classes: ["merge_right_button", "btn", "btn-outline-secondary"],
			parent: this.segment_button_group,
			properties: { innerText: "Merge →" },
		});
		this.merge_right_button.addEventListener("click", (e) =>
			this.merge_segments(e, "right")
		);

		this.add_marker_button = createNewElement({
			type: "button",
			classes: ["add_marker_button", "btn", "btn-outline-secondary"],
			parent: this.segment_button_group,
			properties: { innerText: "Add Marker" },
		});
		this.add_marker_button.addEventListener("click", (e) =>
			this.add_marker(e)
		);

		this.presence_slider_container = createNewElement({
			type: "div",
			classes: ["presence_slider_container"],
			parent: this.segment_editing_container,
			properties: {},
		});
		this.presence_slider_start = createNewElement({
			type: "input",
			classes: ["presence_slider", "presence_slider_start", "form-range"],
			parent: this.presence_slider_container,
			properties: { type: "range", min: 0, max: GLOBAL_presence_scale },
		});
		this.presence_slider_start.addEventListener("input", (e) =>
			this.change_opacity(e, "start")
		);
		this.presence_slider_end = createNewElement({
			type: "input",
			classes: ["presence_slider", "presence_slider_end", "form-range"],
			parent: this.presence_slider_container,
			properties: {
				type: "range",
				min: 0,
		...
		this.presence_slider_end.addEventListener("input", (e) =>
			this.change_opacity(e, "end")
		);
		this.presence_slider_independent_toggle = createNewElement({
			type: "input",
			classes: ["presence_slider_independent_toggle", "form-check-input"],
			parent: this.presence_slider_container,
			properties: { type: "checkbox" },
		});
		this.presence_slider_independent_toggle.addEventListener(
			"change",
			(e) => {
	...
	dragging_handler(e) {
		...
		if (afterElement === -1) {
			this.all_layer_containers.insertBefore(
				draggable,
				this.all_layer_containers.firstChild
			);
			...
			const afterElement_id =
				layers_not_dragging[afterElement].layer_data.layer_id;
	...
	zoom_handler(zoom_type) {
		...
		this.seek_slider.style.width =
			(this.file_length / this.length_padding) * this.scale + "px";
	}
			...
			for (let j = 0; j < this.layers[i].segment_array.length; j++) {
				if (
					...
					if (this.presence_slider_end.disabled === true) {
						let color_value = this.layers[i].segment_array[
							j
						].data.color
							.split(", ")[1]
							.slice(0, -4);
						let new_color_value =
							color_value + new_saturation_value + ")";
						formated_color_value =
							...
							new_color_value +
							")";
						this.layers[i].segment_array[j].data.start_presence =
	...
	start_audio_file_activity_setup(sent_url) {
		...
		this.open_file_trigger_button = createNewElement({
			type: "button",
			classes: ["interface_button"],
			parent: document.body,
			properties: { innerText: "Choose Audio File 2" },
		});
		this.open_file_trigger_button.addEventListener("click", () =>
			this.open_audio_button.click()
		);
		this.loaded_file_name_label = createNewElement({
			type: "div",
			classes: ["interface_button"],
			parent: document.body,
			properties: { innerText: "(no audio file loaded)" },
		});
	...
	get_user_audio_file(sent_url) {
		...
		if (developing === true && location.hostname.includes("localhost")) {
			...
			this.uploaded_audio.src =
				"http://192.168.1.111/bri_former_server_sample/sheep_may_safely_graze.mp3";
	...
	uploaded_audio_loadedmetadata_handler() {
		...
		if (developing === true && location.hostname.includes("localhost")) {
			...
			this.loaded_file_name_label.innerText =
				this.open_audio_button.files[0].name;
		}
	...
	move_seek_slider_with_audio_position(sender) {
		...
		switch (this.activity_type) {
			case "audio_file":
				...
				this.seek_slider.value = parseInt(
					this.uploaded_audio.currentTime * this.audio_speed
				);
				break;
			case "youtube_link":
				this.seek_slider.value = parseInt(
					playerx.getCurrentTime() * this.audio_speed
				);
				this.slider_position = parseInt(playerx.getCurrentTime());
				console.log(
					"playerx.getCurrentTime(): " + playerx.getCurrentTime()
				);
		...
		for (let i = 0; i < this.layers.length; i++) {
			for (let j = 0; j < this.layers[i].segment_array.length; j++) {
				if (
					...
					let formated_color_value;
					let color_value = this.layers[i].segment_array[j].data.color
						.split(", ")[1]
						.slice(0, -4);
					let new_color_value =
						color_value + new_saturation_value + ")";
