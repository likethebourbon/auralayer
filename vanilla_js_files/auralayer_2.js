...
class Layer {
    constructor(sent_container, sent_layer_data, sent_file_length, sent_parent, sent_mode) {
        ...
        if (typeof this.layer_data.show_in_table === 'undefined') {
            this.layer_data.show_in_table = true
        }

        if (typeof this.layer_data.hide_name_in_diagram === 'undefined') {
            this.layer_data.hide_name_in_diagram = false
        }
    ...
    initialize() {
        ...
        this.layer_settings_button = createNewElement({
            ...
            styles: { display: 'none' },
            attributes: { title: 'Layer Settings' },
            events: {
                click: (e) => {
        ...
        this.duplicate_layer_button = createNewElement({
            type: 'button',
            classes: ['duplicate_layer_button', 'layer_controls_elements', 'btn', 'btn-secondary'],
            parent: this.layer_settings_container,
            properties: { innerHTML: `<i class="bi bi-copy"></i>` },
            attributes: { title: 'Duplicate Layer' },
        })
        ...
        this.delete_layer_button = createNewElement({
            type: 'button',
            classes: ['delete_layer_button', 'layer_controls_elements', 'btn', 'btn-secondary'],
            parent: this.layer_settings_container,
        ...
        this.name_edit_button = createNewElement({
            type: 'button',
            classes: ['name_edit_button', 'layer_controls_elements', 'btn', 'btn-secondary'],
            parent: this.layer_settings_container,
            properties: { innerHTML: `<i class="bi bi-pen"></i>` },
            attributes: { title: 'Edit Name' },
        ...
        this.layer_texture_picker = createNewElement({
            type: 'button',
            classes: ['layer_texture_picker', 'layer_controls_elements', 'btn', 'btn-secondary'],
            parent: this.layer_settings_container,
            properties: { innerHTML: `<i class="bi bi-bricks"></i>` },
            attributes: { title: 'Texture Picker' },
        })
        ...
        for (let i = 0; i < Object.keys(textures).length; i++) {
            ...
            let texture = createNewElement({
                type: 'button',
                classes: ['shape_background_texture', 'shape_background_texture_' + (i + 1)],
                parent: this.texture_selector,
                styles: { background: 'url(images/' + each_value + ')' },
                properties: { title: each_key },
            })
        ...
        this.hide_layer_from_data_table_container = createNewElement({
            type: 'div',
            classes: ['hide_layer_from_data_table_container', 'form-check', 'form-switch'],
            parent: this.layer_settings_container,
            properties: {},
            attributes: { title: 'Hide from data table toggle' },
        })
        this.hide_layer_from_data_table_input = createNewElement({
            type: 'input',
            classes: ['hide_layer_from_data_table_input', 'form-check-input'],
            parent: this.hide_layer_from_data_table_container,
            properties: {},
            attributes: { type: 'checkbox', role: 'switch', id: 'hide_layer_from_data_table_input', checked: true },
        })
        this.hide_layer_from_data_table_label = createNewElement({
            type: 'label',
            classes: ['hide_layer_from_data_table_label', 'form-check-label'],
            parent: this.hide_layer_from_data_table_container,
            properties: { innerText: 'Data Table' },
            attributes: { for: 'hide_layer_from_data_table_input' },
        })
        ...
        if (this.layer_data.show_in_table === false) {
            this.hide_layer_from_data_table_input.checked = false
        }

        // -----------------------------------
        //      NAME ON DIAGRAM TOGGLE
        // -----------------------------------
        this.hide_name_on_diagram_toggle = createNewElement({
            type: 'div',
            classes: ['hide_name_on_diagram_toggle', 'form-check', 'form-switch'],
            parent: this.layer_settings_container,
            properties: {},
            attributes: { title: 'Hide name in diagram toggle' },
        })
        this.hide_name_on_diagram_input = createNewElement({
            type: 'input',
            classes: ['hide_name_on_diagram_input', 'form-check-input'],
            parent: this.hide_name_on_diagram_toggle,
            properties: {},
            attributes: { type: 'checkbox', role: 'switch', id: 'hide_name_on_diagram_input', checked: false },
        })
        this.hide_name_on_diagram_label = createNewElement({
            type: 'label',
            classes: ['hide_name_on_diagram_label', 'form-check-label'],
            parent: this.hide_name_on_diagram_toggle,
            properties: { innerText: 'Hide Name' },
            attributes: { for: 'hide_name_on_diagram_input' },
        })
    ...
    create_segment(start, end = -1, start_presence = -1, end_presence = -1, presence_sync, sent_segment = {}) {
        ...
        if (this.mode === 'load_existing_layer') {
            this.segment_array.push(
                new Segment(
                    this,
                    this.current_segment_index_2,
                    end,
                    JSON.parse(JSON.stringify(this.layer_data.segments)),
                    sent_segment.color,
            ...
            if (premature_exit === false) {
                ...
                this.segment_array[this.current_segment_index].segment.style.width =
                    (this.layer_data.segments[this.current_segment_index].end_pos / this.parent.resolution - this.layer_data.segments[this.current_segment_index].start_pos / this.parent.resolution) * this.parent.scale +
...
class Segment {
    ...
    create_segment() {
        ...
        this.SegmentTimestampInputBoxButtonIncreaseBeginning = createNewElement({
            type: 'button',
            classes: ['SegmentTimestampInputBoxButtonIncreaseBeginning', 'btn', 'btn-primary'],
            parent: this.SegmentTimestampInputBox,
        ...
        this.SegmentTimestampInputBoxButtonDecreaseBegining = createNewElement({
            type: 'button',
            classes: ['SegmentTimestampInputBoxButtonDecreaseBegining', 'btn', 'btn-primary'],
            parent: this.SegmentTimestampInputBox,
            properties: { innerText: '↓' },
        })
        ...
        this.SegmentTextInput = createNewElement({
            ...
            dataset: { text_value: this.data.text[0].inner_text },
        })
        ...
        this.SegmentPresenceStartRange = createNewElement({
            ...
            properties: { type: 'range', innerText: this.data.start_presence, min: '0', max: GLOBAL_presence_scale, value: this.data.start_presence },
            events: { input: (e) => this.SegmentPresenceStartRangeHandler(e, 'start') },
        })
        ...
        this.SegmentPresenceEndRange = createNewElement({
            ...
            properties: { type: 'range', innerText: this.data.end_presence, min: '0', max: GLOBAL_presence_scale, value: this.data.end_presence, disabled: this.data.presence_sync },
            events: { input: (e) => this.SegmentPresenceStartRangeHandler(e, 'end') },
        })
        ...
        this.TextEditingLeftAlignButton = createNewElement({
            type: 'button',
            classes: ['TextEditingLeftAlignButton', 'btn', 'btn-outline-secondary', 'border-0'],
            parent: this.TextEditingMenuContainer_SingleSegment,
            properties: { innerHTML: `<i class="bi-justify-left"></i>` },
            events: {
                click: (e) => {
                    this.ChangeTextFormat({ style: 'textAlign', value: 'left' })
                },
            },
        })
        this.TextEditingCenterAlignButton = createNewElement({
            type: 'button',
            classes: ['TextEditingCenterAlignButton', 'btn', 'btn-outline-secondary', 'border-0'],
            parent: this.TextEditingMenuContainer_SingleSegment,
            properties: { innerHTML: `<i class="bi-justify"></i>` },
            events: {
                click: (e) => {
                    this.ChangeTextFormat({ style: 'textAlign', value: 'center' })
                },
            },
        })
        this.TextEditingRightAlignButton = createNewElement({
            type: 'button',
            classes: ['TextEditingRightAlignButton', 'btn', 'btn-outline-secondary', 'border-0'],
            parent: this.TextEditingMenuContainer_SingleSegment,
            properties: { innerHTML: `<i class="bi-justify-right"></i>` },
            events: {
                click: (e) => {
                    this.ChangeTextFormat({ style: 'textAlign', value: 'right' })
                },
            },
        })
        this.TextEditingBoldButton = createNewElement({
            type: 'button',
            classes: ['TextEditingBoldButton', 'btn', 'btn-outline-secondary', 'border-0'],
            parent: this.TextEditingMenuContainer_SingleSegment,
            properties: { innerHTML: `<i class="bi-type-bold"></i>` },
            events: {
                click: (e) => {
                    this.ChangeTextFormat({ style: 'fontWeight', value: 'bold' })
                },
            },
        })
        this.TextEditingItalicButton = createNewElement({
            type: 'button',
            classes: ['TextEditingItalicButton', 'btn', 'btn-outline-secondary', 'border-0'],
            parent: this.TextEditingMenuContainer_SingleSegment,
            properties: { innerHTML: `<i class="bi-type-italic"></i>` },
            events: {
                click: (e) => {
                    this.ChangeTextFormat({ style: 'fontStyle', value: 'italic' })
                },
            },
        })
        this.TextEditingUnderlineButton = createNewElement({
            type: 'button',
            classes: ['TextEditingUnderlineButton', 'btn', 'btn-outline-secondary', 'border-0'],
            parent: this.TextEditingMenuContainer_SingleSegment,
            properties: { innerHTML: `<i class="bi-type-underline"></i>` },
            events: {
                click: (e) => {
                    this.ChangeTextFormat({ style: 'textDecoration', value: 'underline' })
                },
            },
        })
        this.TextEditingStrikeThroughButton = createNewElement({
            type: 'button',
            classes: ['TextEditingStrikeThroughButton', 'btn', 'btn-outline-secondary', 'border-0'],
            parent: this.TextEditingMenuContainer_SingleSegment,
            properties: { innerHTML: `<i class="bi-type-strikethrough"></i>` },
            events: {
                click: (e) => {
                    this.ChangeTextFormat({ style: 'textDecoration', value: 'line-through' })
                },
            },
        })
        this.TextEditingFontSizeIncreaseButton = createNewElement({
            type: 'button',
            classes: ['TextEditingFontSizeIncreaseButton', 'btn', 'btn-outline-secondary', 'border-0'],
            parent: this.TextEditingMenuContainer_SingleSegment,
            properties: { innerHTML: `A+` },
            events: {
                click: (e) => {
                    this.ChangeTextFormat({ style: 'fontSize', type: 'increase' })
                },
            },
        })
        this.TextEditingFontSizeDecreaseButton = createNewElement({
            type: 'button',
            classes: ['TextEditingFontSizeDecreaseButton', 'btn', 'btn-outline-secondary', 'border-0'],
            parent: this.TextEditingMenuContainer_SingleSegment,
            properties: { innerHTML: `A-` },
            events: {
                click: (e) => {
                    this.ChangeTextFormat({ style: 'fontSize', type: 'decrease' })
                },
            },
        })
        ...
        this.segment_text_1.addEventListener('click', (e) => {
            this.segment_text_click_handler(e)
        })
    ...
    AdjustTimeStamp(sent_direction, sent_location) {
        ...
        if (left_segment.data.start_pos < 0) {
            left_segment.data.start_pos = 0
        }
        if (right_segment.data.end_pos > this.parent.parent_file_length) {
            right_segment.data.end_pos = this.parent.parent_file_length
        }
...
class Auralayer {
    ...
    create_activity_selection_interface() {
        ...
        this.NewAuralayerFromYoutubeButton = createNewElement({
            type: 'button',
            classes: ['NewAuralayerFromYoutubeButton', 'btn', 'btn-primary'],
            parent: this.ActivitySelectionBody,
            properties: { innerText: 'Create with YouTube link' },
        ...
        this.NewAuralayerFromAudioFileButton = createNewElement({
            type: 'button',
            classes: ['NewAuralayerFromAudioFileButton', 'btn', 'btn-primary'],
            parent: this.ActivitySelectionBody,
            properties: { innerText: 'Create with local audio file' },
            events: { click: (e) => this.StartAudioFileActivitySetup('nothing') },
        })
        ...
        this.OpenExistingAuralayerFromFileButton = createNewElement({
            type: 'button',
            classes: ['OpenExistingAuralayerFromFileButton', 'btn', 'btn-primary'],
            parent: this.ActivitySelectionBody,
            properties: { innerText: 'Load .auralayer file' },
            events: { click: (e) => this.ImportFromFile.click() },
        })
        this.NewAuralayerFromAudioFileWithAbsoluteURL_Container = createNewElement({
            type: 'div',
            classes: ['NewAuralayerFromAudioFileWithAbsoluteURL_Container', 'ActivityButtonContainer'],
            parent: this.ActivitySelectionBody,
            styles: { display: 'none' },
        })
        this.NewAuralayerFromAudioFileWithAbsoluteURL_Button = createNewElement({
            type: 'button',
            classes: ['NewAuralayerFromAudioFileWithAbsoluteURL_Button', 'btn', 'btn-primary'],
            parent: this.ActivitySelectionBody,
            properties: { innerText: 'Create with absolute URL' },
            events: { click: () => this.StartAudioFileActivitySetup() },
        })
        ...
        if (location.hostname.includes('localhost')) {
            this.NewAuralayerFromAudioFileWithAbsoluteURL_Container.style.display = 'flex'
        }
    }
    initialize_interface() {
        ...
        this.TextEditingLeftAlignButton.addEventListener('click', (e) => {
            this.ChangeTextFormat({ style: 'textAlign', value: 'left' })
        })
        ...
        this.TextEditingCenterAlignButton.addEventListener('click', (e) => {
            this.ChangeTextFormat({ style: 'textAlign', value: 'center' })
        })
        ...
        this.TextEditingRightAlignButton.addEventListener('click', (e) => {
            this.ChangeTextFormat({ style: 'textAlign', value: 'right' })
        })
        ...
        this.TextEditingBoldButton.addEventListener('click', (e) => {
            this.ChangeTextFormat({ style: 'fontWeight', value: 'bold' })
        })
        ...
        this.TextEditingItalicButton.addEventListener('click', (e) => {
            this.ChangeTextFormat({ style: 'fontStyle', value: 'italic' })
        })
        ...
        this.TextEditingStrikeThroughButton.addEventListener('click', (e) => {
            this.ChangeTextFormat({ style: 'textDecoration', value: 'line-through' })
        })
        ...
        this.TextEditingFontSizeIncreaseButton.addEventListener('click', (e) => {
            this.ChangeTextFormat({ style: 'fontSize', type: 'increase' })
        })
        ...
        this.TextEditingFontSizeDecreaseButton.addEventListener('click', (e) => {
            this.ChangeTextFormat({ style: 'fontSize', type: 'decrease' })
        })
        ...
        this.SearchTableInput.addEventListener('focus', (e) => {
            this.in_text_editor = true
        })
            ...
            if (this.example_data.layers.length > 0) {
                ...
                if (this.save_position === this.save_array.length) {
                    ...
                    this.save_array[this.save_position] = {
                        program_data: { color_count: this.color_count, layer_id_pos: this.layer_id_pos, slider_thumb_offset: this.slider_thumb_offset, slider_thumb_height: this.slider_thumb_height },
                        layer_data: copy,
                    }
                    // console.log(this.save_array);
        ...
        for (let i = 0; i < this.layers.length; i++) {
            for (let j = 0; j < this.layers[i].segment_array.length; j++) {
                this.layers[i].segment_array[j].segment.style.width =
                    (this.layers[i].segment_array[j].data.end_pos / this.resolution - this.layers[i].segment_array[j].data.start_pos / this.resolution) * this.scale + this.scale / this.resolution - 1 + 'px'
    ...
    StartYoutubeActivitySetup() {
        ...
        this.url_prompt_box_top = createNewElement({
            type: 'div',
            classes: ['url_prompt_box_top', 'modal-header', 'd-flex', 'justify-content-between', 'mb-4'],
            parent: this.url_prompt_box_container,
            properties: { innerHTML: "<h5 class='modal-title'>Create with Youtube link</h5>" },
        })
        ...
        this.url_prompt_input_box = createNewElement({
            type: 'input',
            classes: ['form-control'],
            parent: this.url_prompt_box_middle,
            properties: { id: 'url_prompt_input_box', innerText: 'Paste a YouTube URL below', type: 'text', placeholder: 'Paste URL here' },
        })
    ...
    StartAudioFileActivitySetup(sent_url) {
        ...
        if (sent_url === 'nothing') {
            this.audio_file_prompt_backdrop = createNewElement({ type: 'div', classes: ['audio_file_prompt_backdrop'], parent: document.body })
            this.audio_file_prompt_box_container = createNewElement({ type: 'div', classes: ['audio_file_prompt_box_container', 'border-rounded'], parent: document.body })
            this.audio_file_prompt_box_top = createNewElement({
                type: 'div',
                classes: ['audio_file_prompt_box_top', 'modal-header', 'd-flex', 'justify-content-between', 'mb-4'],
                parent: this.audio_file_prompt_box_container,
                properties: { innerHTML: "<h5 class='modal-title'>Create with local audio file</h5>" },
            })
            this.audio_file_prompt_box_middle = createNewElement({ type: 'div', classes: ['audio_file_prompt_box_middle'], parent: this.audio_file_prompt_box_container })
            this.open_audio_button = createNewElement({
                type: 'input',
                classes: ['open_audio_button', 'form-control'],
                parent: this.audio_file_prompt_box_middle,
                properties: { type: 'file', name: 'open_audio_button' },
                styles: { display: 'block' },
                events: { change: () => this.get_user_audio_file('nothing') },
            })
            this.cancel_opening_audio_button = createNewElement({ type: 'button', classes: ['cancel_opening_audio_button', 'btn-close'], parent: this.audio_file_prompt_box_top, events: { click: (e) => this.cancel_opening_audio_button_handler() } })
            this.audio_file_prompt_box_bottom = createNewElement({ type: 'div', classes: ['audio_file_prompt_box_bottom'], parent: this.audio_file_prompt_box_container })
        }
    ...
    start_program_after_media_loaded() {
        ...
        if (this.example_data.layers.length === 0) {
            ...
            let initial_layer_data = {
                ...
                color: 'linear-gradient(to right, ' + random_color + ', ' + random_color + ')',
                segments: [],
                markers: [],
                layer_id_pos: 0,
                show_in_table: true,
                hide_name_in_diagram: false,
            }
    ...
    add_layer_handler() {
        ...
        let new_initial_layer_data = {
            ...
            hide_name_in_diagram: this.hide_name_in_diagram,
        }
    ...
    create_shareable_link() {
        ...
        qr_code_script.addEventListener('load', () => {
            ...
            let date = new Date()
            let time_stamp = date.getUTCFullYear() + '_' + (date.getUTCMonth() + 1) + '_' + date.getUTCDate() + '_' + date.getUTCHours() + '_' + date.getUTCMinutes() + '_' + date.getUTCMilliseconds()
            ...
            debugger
            if (this.activity_type == 'audio_file' && !((this.url_activity_text == 'load_from_web_link_audio_file') || this.url_activity_text == 'open_from_web_link_audio_file2')) {
...
function download_image() {
    ...
    scriptPromise.then(() => {
        ...
        for (let each of all_shapes) {
            if (each.getBoundingClientRect().right > right_most) {
                right_most = each.getBoundingClientRect().right
            }
        }
